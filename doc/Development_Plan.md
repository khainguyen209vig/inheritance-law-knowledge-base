...
- Trang gốc `/` chuyển hướng tới `/guided`; kế hoạch chuyển đổi sang hội thoại có hướng dẫn được tách tại `doc/Development_Plan_Guided_Conversation.md`. `/cases` và `/modules` được giữ làm giao diện quản lý/chế độ kỹ thuật; danh sách và trang chi tiết đều hỗ trợ xóa hồ sơ có xác nhận.
