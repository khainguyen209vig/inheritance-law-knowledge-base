...
import { edgeLabel, graphDiagnostics, graphLevels, graphWarnings, type FamilyEdgeType, type FamilyGraph, type FamilyPerson, type StepCareStatus } from "@/modules/family-graph/model";
...
const edgeTypes: Array<{ type: FamilyEdgeType; label: string; help: string }> = [
  ...
  { type: "step-parent-of", label: "Bố dượng/mẹ kế → con riêng", help: "Quan hệ đặc biệt cần đánh giá chăm sóc riêng" },
...
export function FamilyGraphEditor({ graph, selectedId, onSelect, onChange }: { graph: FamilyGraph; selectedId?: string; onSelect: (id: string) => void; onChange: (graph: FamilyGraph) => void }) {
  ...
  const [stepCareStatus, setStepCareStatus] = useState<StepCareStatus>();
  ...
  const warnings = useMemo(() => graphWarnings(graph), [graph]);
  ...
  function addEdge() {
    ...
    if (duplicate) return;
    onChange({ ...graph, edges: [...graph.edges, { id: `edge-${Date.now().toString(36)}`, from: sourceId, to: targetId, type: edgeType, careStatus: edgeType === "step-parent-of" ? stepCareStatus : undefined }] });
    setSourceId(undefined); setTargetId(undefined);
    setStepCareStatus(undefined);
  }

  return <div className="space-y-4">
    <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-2"><div><CardTitle>Cây/graph quan hệ</CardTitle><CardDescription>Chọn node để sửa người. Hàng thừa kế không được nhập tại đây.</CardDescription></div><Badge variant="secondary">{graph.people.length} người · {graph.edges.length} cạnh</Badge></div></CardHeader><CardContent className="space-y-4"><div className="overflow-x-auto rounded-xl border bg-muted/20 p-4"><div className="flex min-w-[620px] flex-col gap-5">{levelGroups.map(([level, people]) => <div key={String(level)}><div className="mb-2 flex items-center gap-2"><span className="h-px flex-1 bg-border" /><span className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{levelLabel(level)}</span><span className="h-px flex-1 bg-border" /></div><div className="flex flex-wrap justify-center gap-3">{people.map((person) => <button key={person.id} type="button" onClick={() => onSelect(person.id)} className={cn("min-w-36 rounded-xl border bg-card p-3 text-left shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-ring", selectedId === person.id && "border-primary ring-1 ring-primary", person.id === graph.deceasedId && "border-amber-400 bg-amber-50")}><span className="block text-xs text-muted-foreground">{person.id === graph.deceasedId ? "Người để lại di sản" : person.life === "dead-before-or-same" ? "Đã chết trước/cùng lúc" : person.life === "alive" ? "Còn sống" : "Chưa rõ trạng thái"}</span><span className="mt-1 block font-semibold">{person.name}</span><span className="mt-1 block font-mono text-[9px] text-muted-foreground">{person.id}</span></button>)}</div></div>)}</div></div>{diagnostics.length ? <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-800">{diagnostics.map((item) => <p key={item}>• {item}</p>)}</div> : <p className="text-xs text-emerald-700">Graph không có cạnh trùng, cạnh tự nối hoặc chu trình cha/mẹ–con.</p>}{warnings.length ? <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">{warnings.map((item) => <p key={item}>• {item}</p>)}</div> : null}</CardContent></Card>
        <Card><CardHeader><CardTitle>Nối hai người</CardTitle><CardDescription>Ba bước tạo trực tiếp một fact quan hệ. Mũi tên của quan hệ cha/mẹ luôn đi từ cha/mẹ tới con.</CardDescription></CardHeader><CardContent className="space-y-4"><GraphStep number="1" title="Chọn người thứ nhất"><PersonChoices people={graph.people} selected={sourceId} excluded={targetId} onSelect={setSourceId} /></GraphStep><GraphStep number="2" title="Chọn loại cạnh"><div className="grid gap-2 sm:grid-cols-2">{edgeTypes.map((item) => <button key={item.type} type="button" onClick={() => { setEdgeType(item.type); if (item.type !== "step-parent-of") setStepCareStatus(undefined); }} className={cn("rounded-lg border p-3 text-left text-sm", edgeType === item.type && "border-primary bg-primary/[0.06]")}><span className="font-medium">{item.label}</span><span className="mt-1 block text-[10px] text-muted-foreground">{item.help}</span></button>)}</div></GraphStep><GraphStep number="3" title="Chọn người thứ hai"><PersonChoices people={graph.people} selected={targetId} excluded={sourceId} onSelect={setTargetId} /></GraphStep>{edgeType === "step-parent-of" ? <GraphStep number="4" title="Đánh giá quan hệ chăm sóc, nuôi dưỡng (có thể bổ sung sau)"><p className="mb-2 text-xs text-muted-foreground">Chọn theo thông tin đã được rà soát. Để trống nếu chưa đủ căn cứ; CLIPS sẽ báo thiếu dữ kiện thay vì tự coi là “không có chăm sóc”.</p><div className="grid gap-2 sm:grid-cols-2"><CareChoice selected={stepCareStatus === "established"} onClick={() => setStepCareStatus("established")}>Có, như cha con/mẹ con</CareChoice><CareChoice selected={stepCareStatus === "not-established"} onClick={() => setStepCareStatus("not-established")}>Đã rà soát: không có</CareChoice></div></GraphStep> : null}<div className="flex items-center justify-between gap-3 rounded-lg bg-muted/40 p-3"><p className="text-xs text-muted-foreground">{sourceId && targetId ? edgePreview(edgeType, peopleById.get(sourceId), peopleById.get(targetId)) : "Chọn đủ hai người để xem fact sẽ tạo."}</p><Button disabled={!sourceId || !targetId} onClick={addEdge}>Thêm cạnh</Button></div></CardContent></Card>
    <Card><CardHeader><CardTitle>Danh sách cạnh</CardTitle><CardDescription>Mỗi dòng tương ứng một asserted fact được gửi vào CLIPS.</CardDescription></CardHeader><CardContent className="space-y-2">{graph.edges.length ? graph.edges.map((edge) => <div key={edge.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border p-3"><div><p className="text-sm font-medium">{edgeLabel(edge, peopleById)}</p><code className="text-[10px] text-muted-foreground">{edge.type}({edge.from}, {edge.to})</code>{edge.type === "step-parent-of" ? <div className="mt-2 flex flex-wrap gap-1"><Button variant={edge.careStatus === "established" ? "secondary" : "outline"} size="sm" onClick={() => updateCareStatus(graph, edge.id, "established", onChange)}>Có chăm sóc</Button><Button variant={edge.careStatus === "not-established" ? "secondary" : "outline"} size="sm" onClick={() => updateCareStatus(graph, edge.id, "not-established", onChange)}>Không có sau rà soát</Button></div> : null}</div><Button variant="ghost" size="sm" onClick={() => onChange({ ...graph, edges: graph.edges.filter((item) => item.id !== edge.id) })}>Xóa cạnh</Button></div>) : <p className="rounded-lg border border-dashed p-4 text-center text-sm text-muted-foreground">Chưa có cạnh quan hệ.</p>}</CardContent></Card>
  </div>;
}
...
function CareChoice({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: ReactNode }) { return <button type="button" onClick={onClick} className={cn("rounded-lg border p-3 text-left text-sm", selected && "border-primary bg-primary/[0.06]")}>{children}</button>; }
function updateCareStatus(graph: FamilyGraph, edgeId: string, careStatus: StepCareStatus, onChange: (graph: FamilyGraph) => void) { onChange({ ...graph, edges: graph.edges.map((edge) => edge.id === edgeId ? { ...edge, careStatus } : edge) }); }
function edgePreview(type: FamilyEdgeType, from?: FamilyPerson, to?: FamilyPerson) { if (!from || !to) return ""; if (type === "spouse-at-opening") return `${from.name} — vợ/chồng — ${to.name}`; if (type === "step-parent-of") return `${from.name} — bố dượng/mẹ kế của → ${to.name}`; return `${from.name} — ${type === "biological-parent-of" ? "cha/mẹ đẻ của" : "cha/mẹ nuôi của"} → ${to.name}`; }
