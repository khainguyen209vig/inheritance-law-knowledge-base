...
export function GuidedConclusion({ state }: { state: GuidedCaseState }) {
  ...
  const message = resolution === "conflict"
    ...
    : resolution === "unknown"
      ? "Hệ thống đã đối chiếu các thông tin hiện có nhưng vẫn chưa đủ căn cứ để chọn một kết luận xác định. Trạng thái này khác với việc còn một câu hỏi cụ thể chưa được trả lời."
  ...
  return <div className="space-y-4">
    ...
    <Card className="max-w-3xl"><CardHeader><div className="flex flex-wrap items-center justify-between gap-2"><Badge variant={status === "complete" ? "success" : status === "conflict" ? "destructive" : "warning"}>{badge}</Badge><span className="text-xs text-muted-foreground">Kết quả đối chiếu quy tắc pháp lý</span></div><CardTitle className="pt-2">Kết quả rà soát</CardTitle><CardDescription>Kết luận chỉ áp dụng cho thông tin đã cung cấp và các điều luật đã được mô hình hóa trong đồ án.</CardDescription></CardHeader><CardContent className="space-y-3">
