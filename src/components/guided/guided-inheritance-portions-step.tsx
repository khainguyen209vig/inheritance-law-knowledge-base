...
export function GuidedInheritancePortionsStep({ state, onStateChange }: { state: GuidedCaseState; onStateChange: (state: GuidedCaseState) => void }) {
  ...
  return <div className="max-w-3xl space-y-4">
    <Card><CardHeader><Badge variant="outline" className="w-fit">Goal · inheritance-regime</Badge><CardTitle className="text-lg">Các phần di sản được xử lý thế nào?</CardTitle><CardDescription>Khai riêng từng phần vì một hồ sơ có thể đồng thời có phần chia theo di chúc và phần chia theo pháp luật.</CardDescription></CardHeader><CardContent>{hasWill === false ? <p className="rounded-lg bg-muted/40 p-3 text-sm">Bạn đã xác nhận không có di chúc. CLIPS sẽ áp dụng R-A01 cho các phần được khai.</p> : willValidity === "false" ? <p className="rounded-lg bg-muted/40 p-3 text-sm">Di chúc đã được xác định không hợp lệ. CLIPS sẽ kiểm tra R-A02 cho từng phần.</p> : blockedByWill ? <p className="rounded-lg bg-amber-50 p-3 text-sm text-amber-900">Chưa có kết luận rõ ràng về di chúc nên chưa thể mô tả việc định đoạt.</p> : null}</CardContent></Card>
    ...
    <div className="flex justify-end"><Button disabled={!ready || isPending || typeof hasWill !== "boolean"} onClick={saveAndContinue}>{isPending ? "Đang lưu và điều phối CLIPS…" : "Lưu và tiếp tục suy luận"}</Button></div>
