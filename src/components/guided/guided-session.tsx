...
export function GuidedSession({ initialState }: GuidedSessionProps) {
  ...
  return <main className="mx-auto min-h-screen max-w-4xl px-4 py-8 sm:px-6">
    ...
    <section className="mt-8 space-y-5">
      ...
      {editableAnswers.length ? <Card className="max-w-2xl"><CardHeader><CardTitle className="text-base">Thông tin đã cung cấp</CardTitle><CardDescription>Bạn có thể sửa một câu trả lời. Hệ thống sẽ hỏi lại những bước bị ảnh hưởng và cập nhật kết quả theo thông tin mới.</CardDescription></CardHeader><CardContent className="space-y-2">{editableAnswers.map((answer) => <div key={answer.questionId} className="flex items-start justify-between gap-3 rounded-lg border p-3"><div><p className="text-sm text-muted-foreground">{answer.resolution.prompt}</p><p className="mt-1 text-sm font-semibold">{guidedAnswerLabel(answer.resolution, answer.value)}</p></div><Button type="button" size="sm" variant="ghost" onClick={() => { setError(undefined); setEditingQuestionId(answer.questionId); }}>Sửa</Button></div>)}</CardContent></Card> : null}
