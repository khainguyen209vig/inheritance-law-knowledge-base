...
export function GuidedLimitationTimelineStep({ state, onStateChange }: { state: GuidedCaseState; onStateChange: (state: GuidedCaseState) => void }) {
  ...
  return <Card className="max-w-3xl"><CardHeader><Badge variant="outline" className="w-fit">Điều 623 · Timeline</Badge><CardTitle className="text-lg">Bạn muốn xác định thời hiệu cho yêu cầu nào?</CardTitle><CardDescription>CLIPS chọn thời hạn 3, 10 hoặc 30 năm; hệ thống chỉ cộng lịch từ ngày mở thừa kế sau khi luật phù hợp đã được kích hoạt.</CardDescription></CardHeader><CardContent className="space-y-6">
