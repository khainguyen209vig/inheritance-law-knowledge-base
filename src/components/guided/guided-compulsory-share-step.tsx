...
export function GuidedCompulsoryShareStep({ state, onStateChange }: { state: GuidedCaseState; onStateChange: (state: GuidedCaseState) => void }) {
  ...
  return <div className="max-w-3xl space-y-4">
    <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-2"><Badge variant="outline">Điều 644 · rà soát theo người</Badge><Button variant="ghost" size="sm" onClick={() => setSelectedRule("R-F01a")}>Đọc căn cứ Điều 644</Button></div><CardTitle className="text-lg">Xác định nhóm được bảo vệ</CardTitle><CardDescription>Cha, mẹ và vợ/chồng được nhận diện từ graph. Với con, cần thêm nhóm tuổi và khả năng lao động để CLIPS phân loại.</CardDescription></CardHeader></Card>
