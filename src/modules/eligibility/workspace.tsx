...
type Ground = "clear" | "offense-deceased" | "abuse-deceased" | "support" | "other-heir" | "deception" | "coercion" | "forgery" | "alteration" | "destruction" | "concealment";
interface PersonDraft { id: string; name: string; ground?: Ground; exception: boolean; will?: string }

const grounds: Array<{ id: Ground; title: string; detail: string; ruleId: string }> = [
...
const eligibilityPredicates = new Set([
...
export function EligibilityWorkspace({ module, initialCase }: { module: AnalysisModuleDefinition; initialCase?: InitialCase }) {
  ...
  const initialPeople = useMemo(() => restorePeople(initialCase?.facts ?? [], stableToken, willSubjects[0]), [initialCase?.facts, stableToken, willSubjects]);
  ...
  function update(update: Partial<PersonDraft>) { if (!active) return; setPeople((current) => current.map((person) => person.id === active.id ? { ...person, ...update } : person)); setRun(undefined); }
...
function buildEligibilityFacts(people: PersonDraft[]): ApiFact[] {
  const facts: ApiFact[] = [];
...
function restorePeople(facts: ApiFact[], token: string, defaultWill?: string): PersonDraft[] {
  const ids = [...new Set(facts.flatMap((fact) => fact.predicate === "eligibility-candidate" && fact.subject ? [fact.subject] : []))];
  const people = ids.map((id, index) => { const own = facts.filter((fact) => fact.subject === id); return { id, name: String(own.find((fact) => fact.predicate === "person-label")?.value ?? `Người được xét ${index + 1}`), ground: restoreGround(own), exception: own.some((fact) => fact.predicate === "deceased-knew-disqualifying-act" && fact.value === true), will: String(own.find((fact) => fact.predicate === "eligibility-applicable-will")?.value ?? defaultWill ?? "") || undefined }; });
  return people.length ? people : [{ id: `person-${token}-0`, name: "Người được xét 1", exception: false, will: defaultWill }];
}
function restoreGround(facts: ApiFact[]): Ground | undefined {
...
function groundTitle(ground?: Ground): string { return grounds.find((item) => item.id === ground)?.title ?? "Chưa rà soát"; }
function eligibilityLabel(value: string): string { return value === "not-excluded" ? "Không bị loại trừ theo Điều 621" : value === "excluded" ? "Không có quyền hưởng" : value === "exception-under-will" ? "Ngoại lệ theo di chúc" : "Chưa đủ dữ kiện"; }
