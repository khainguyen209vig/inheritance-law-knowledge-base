...
export function RepresentationWorkspace({ module, initialCase }: { module: AnalysisModuleDefinition; initialCase?: InitialCase }) {
  ...
  return <div className="min-h-screen bg-background">
    <header className="border-b bg-card/80"><div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6"><div><p className="font-semibold">Inheritance Reasoner</p><p className="text-xs text-muted-foreground">{module.title} · R-E01/R-E02 · rà soát graph dùng chung</p></div><div className="flex flex-wrap gap-2"><Button asChild variant="ghost" size="sm"><Link href={`/cases/${initialCase.id}`}>Hồ sơ</Link></Button><Button asChild variant="outline" size="sm"><Link href={`/cases/${initialCase.id}/modules/heir-rank`}>Mở graph quan hệ</Link></Button><Badge variant="warning">TEAM_REVIEW</Badge></div></div></header>
