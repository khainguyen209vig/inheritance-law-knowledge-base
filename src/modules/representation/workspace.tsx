...
interface StepRelation { edgeId: string; stepParent: string; stepChild: string; careStatus?: "established" | "not-established" }
...
export function RepresentationWorkspace({ module, initialCase }: { module: AnalysisModuleDefinition; initialCase?: InitialCase }) {
  ...
  const stepRelations = useMemo(() => discoverStepRelations(initialCase?.facts ?? []), [initialCase?.facts]);
  const stepBasis = useMemo(() => new Set(run?.results.filter((result) => result.predicate === "step-relationship-inheritance-basis" && result.value === "true").map((result) => result.subject) ?? []), [run]);
  const stepExcluded = useMemo(() => new Set(run?.results.filter((result) => result.predicate === "eligible-by-step-relationship" && result.value === "false").map((result) => result.subject) ?? []), [run]);
  const stepConflicts = useMemo(() => new Set(run?.results.filter((result) => result.predicate === "step-care-assessment" && result.value === "conflict").map((result) => result.subject) ?? []), [run]);
  ...
  return <div className="min-h-screen bg-background">
    ...
    <main className="mx-auto grid max-w-6xl gap-5 p-4 sm:p-6 lg:grid-cols-[minmax(0,1fr)_370px]">
      <section className="space-y-4"><Card><CardHeader><Badge variant="outline">Nhánh có khả năng thế vị</Badge><CardTitle className="pt-2">{initialCase.title}</CardTitle><CardDescription>Hệ thống dò đường cha/mẹ–con dài hai hoặc ba cạnh. Chọn người cần xét; việc chọn chỉ tạo marker phạm vi, không khẳng định họ được hưởng.</CardDescription></CardHeader><CardContent className="space-y-3">{paths.length ? paths.map((path) => <button key={`${path.candidate}-${path.generation}`} type="button" onClick={() => toggle(path.candidate)} className={cn("w-full rounded-xl border p-4 text-left", selected.has(path.candidate) && "border-primary bg-primary/[0.05]")}><span className="flex flex-wrap items-center justify-between gap-2"><span className="font-semibold">{labelOf(labels, path.candidate)}</span><Badge variant={selected.has(path.candidate) ? "success" : "secondary"}>{selected.has(path.candidate) ? "Đang xét" : "Bỏ qua"}</Badge></span><span className="mt-2 block text-sm text-muted-foreground">{path.generation === 2 ? "Cháu" : "Chắt"} → thế vị cho {labelOf(labels, path.represented)}</span><code className="mt-2 block overflow-x-auto rounded bg-muted p-2 text-[11px]">{labelOf(labels, path.originalChild)} → {path.generation === 3 ? `${labelOf(labels, path.represented)} → ` : ""}{labelOf(labels, path.candidate)}</code></button>) : <div className="rounded-xl border border-dashed p-6 text-center"><p className="font-medium">Chưa phát hiện nhánh hai hoặc ba thế hệ</p><p className="mt-2 text-sm text-muted-foreground">Graph hiện tại chưa có đủ cạnh cha/mẹ đẻ–con để tạo ứng viên R-E01/R-E02.</p><Button asChild className="mt-4" variant="outline"><Link href={`/cases/${initialCase.id}/modules/heir-rank`}>Kiểm tra graph quan hệ</Link></Button></div>}</CardContent></Card>
        ...
        <Card><CardHeader><CardTitle>Con riêng và bố dượng/mẹ kế</CardTitle><CardDescription>Đánh giá chăm sóc được gắn với từng cạnh quan hệ. Thiếu đánh giá vẫn là chưa biết, không phải “không có”.</CardDescription></CardHeader><CardContent className="space-y-2">{stepRelations.length ? stepRelations.map((relation) => <div key={relation.edgeId} className="rounded-lg border p-3 text-sm"><p><strong>{labelOf(labels, relation.stepParent)}</strong> → {labelOf(labels, relation.stepChild)}</p><Badge className="mt-2" variant={relation.careStatus === "established" ? "success" : relation.careStatus === "not-established" ? "secondary" : "warning"}>{relation.careStatus === "established" ? "Có chăm sóc như cha/mẹ–con" : relation.careStatus === "not-established" ? "Đã xác nhận không có" : "Chưa đánh giá"}</Badge></div>) : <p className="text-sm text-muted-foreground">Chưa có cạnh con riêng–bố dượng/mẹ kế trong graph.</p>}<Button variant="outline" onClick={() => setSelectedRule("R-E04")}>Đọc Điều 654</Button></CardContent></Card>
        ...
        <div className="flex justify-end"><Button disabled={(!selected.size && !hasAdoptionEdges && !stepRelations.length) || isPending} onClick={runInference}>{isPending ? "CLIPS đang suy luận…" : `Chạy suy luận${selected.size ? ` ${selected.size} ứng viên thế vị` : " quan hệ"}`}</Button></div>
      </section>
      <aside><Card className="sticky top-4"><CardHeader><Badge variant="outline">Kết quả mô-đun</Badge><CardTitle className="pt-2">{run ? `${run.results.length} kết quả` : "Chưa chạy CLIPS"}</CardTitle><CardDescription>Quan hệ pháp lý và kết luận thế vị được trình bày tách biệt.</CardDescription></CardHeader><CardContent className="space-y-3">{error ? <p className="rounded-lg bg-red-50 p-3 text-sm text-red-800">{error}</p> : null}{run && adoptionResults.length ? <div className="rounded-xl border border-sky-200 bg-sky-50/50 p-4"><p className="text-sm font-semibold">Căn cứ quan hệ con nuôi</p><div className="mt-2 space-y-2">{adoptionResults.map((result) => <div key={result.subject} className="flex flex-wrap items-center justify-between gap-2 text-sm"><span>{labelOf(labels, result.subject)}</span><div className="flex gap-1"><Badge variant="success">Có căn cứ Điều 653</Badge>{dualParentage.has(result.subject) ? <Badge variant="outline">Có cả quan hệ cha/mẹ đẻ</Badge> : null}</div></div>)}</div><Button className="mt-2 px-0" variant="ghost" size="sm" onClick={() => setSelectedRule("R-E03a")}>Xem căn cứ</Button></div> : null}{run && stepRelations.length ? <div className="rounded-xl border border-violet-200 bg-violet-50/40 p-4"><p className="text-sm font-semibold">Kết quả quan hệ Điều 654</p><div className="mt-2 space-y-2">{stepRelations.map((relation) => { const hasBasis = stepBasis.has(relation.stepChild) && stepBasis.has(relation.stepParent); const excluded = stepExcluded.has(relation.stepChild) && stepExcluded.has(relation.stepParent); const conflict = stepConflicts.has(relation.stepChild); const missing = run.missing.some((item) => item.subject === relation.stepChild && item.predicate === "step-care-status"); return <div key={relation.edgeId} className="rounded-lg border bg-background p-3 text-sm"><p>{labelOf(labels, relation.stepParent)} ↔ {labelOf(labels, relation.stepChild)}</p><Badge className="mt-2" variant={conflict ? "warning" : hasBasis ? "success" : excluded ? "secondary" : "warning"}>{conflict ? "Dữ kiện mâu thuẫn" : hasBasis ? "Có căn cứ thừa kế hai chiều" : excluded ? "Không có căn cứ theo riêng Điều 654" : missing ? "Thiếu đánh giá chăm sóc" : "Chưa có kết luận"}</Badge><Button className="ml-1 mt-2 px-1" variant="ghost" size="sm" onClick={() => setSelectedRule(hasBasis ? "R-E04" : excluded ? "R-E05" : "R-E04")}>Xem căn cứ</Button></div>; })}</div></div> : null}{run ? [...selected].map((person) => { const result = results.get(person); const missing = run.missing.filter((item) => item.subject === person); return <div key={person} className="rounded-xl border p-4"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-semibold">{labelOf(labels, person)}</p><Badge variant={result?.value === "true" ? "success" : result?.value === "false" ? "secondary" : "warning"}>{result?.value === "true" ? "Được thế vị" : result?.value === "false" ? "Không đủ điều kiện" : "Chưa đủ dữ kiện"}</Badge></div>{missing.length ? <p className="mt-2 text-xs text-amber-800">Thiếu: {missing.map((item) => missingLabel(item.predicate)).join("; ")}</p> : null}<div className="mt-2 flex flex-wrap gap-1">{result?.derivations.filter((ruleId) => getRuleExplanation(ruleId)).map((ruleId) => <Button key={ruleId} className="h-7 px-2 text-xs" variant="ghost" size="sm" onClick={() => setSelectedRule(ruleId)}>{ruleId} · xem căn cứ</Button>)}</div></div>; }) : <div className="grid min-h-28 place-items-center rounded-xl border border-dashed text-center text-sm text-muted-foreground">Kết quả và dữ kiện thiếu sẽ xuất hiện ở đây.</div>}{run?.traces.some((trace) => getRuleExplanation(trace.ruleId)?.reviewState === "TEAM_REVIEW") ? <p className="rounded-lg bg-amber-50 p-3 text-xs text-amber-900">Run sử dụng rule TEAM_REVIEW, chưa được phê duyệt pháp lý.</p> : null}</CardContent></Card></aside>
...
function discoverStepRelations(facts: ApiFact[]): StepRelation[] { const statusByEdge = new Map(facts.filter((fact) => fact.predicate === "step-care-status" && fact.subject).map((fact) => [fact.subject as string, fact.value as StepRelation["careStatus"]])); return facts.flatMap((fact): StepRelation[] => fact.predicate === "step-parent-of" && fact.subject && typeof fact.value === "string" ? [{ edgeId: fact.id, stepParent: fact.subject, stepChild: fact.value, careStatus: statusByEdge.get(fact.id) }] : []); }
function labelOf(labels: Map<string, string>, person: string) { return labels.get(person) ?? person; }
function missingLabel(predicate: string) { return ({ "heir-life-status": "tình trạng sống của ứng viên", "article-621-status": "kết quả Điều 621 của ứng viên", "valid-refusal": "trạng thái từ chối", "represented-person-life-status": "tình trạng sống của người được thế vị", "represented-person-article-621-status": "kết quả Điều 621 của người được thế vị", "representation-path-or-qualification": "đường quan hệ hoặc điều kiện của nhánh", "step-care-status": "đánh giá chăm sóc giữa con riêng và bố dượng/mẹ kế" } as Record<string, string>)[predicate] ?? predicate; }
