...
const ownedPredicates = new Set(["compulsory-share-assessment-subject", "age-group", "work-capacity-status", "compulsory-share-calculation", "calculation-person", "calculation-estate-portion", "hypothetical-statutory-share", "testamentary-share-received", "testamentary-share-received-vnd"]);
...
export function CompulsoryShareWorkspace({ module, initialCase }: { module: AnalysisModuleDefinition; initialCase?: InitialCase }) {
  ...
  const initialReceivedVnd = useMemo(() => restoreReceivedVnd(initialCase?.facts ?? []), [initialCase?.facts]);
  ...
  const [receivedVnd, setReceivedVnd] = useState<Record<string, number | undefined>>(initialReceivedVnd);
  ...
  const facts = useMemo(() => [...buildCompulsoryAssessmentFacts(drafts), ...buildCalculationFacts(calculations), ...buildReceivedVndFacts(receivedVnd)], [drafts, calculations, receivedVnd]);
  ...
  const statutoryVndResults = useMemo(() => { const mapped = resultMap(run, "hypothetical-statutory-share-vnd"); const first = mapped.values().next().value; if (first) mapped.set("estate-vnd-calculation", first); return mapped; }, [run]);
  const minimumVndResults = useMemo(() => resultMap(run, "minimum-compulsory-share-vnd"), [run]);
  const shortfallVndResults = useMemo(() => resultMap(run, "compulsory-share-shortfall-vnd"), [run]);
  ...
  function updateReceivedVnd(personId: string, rawValue: string) {
    const value = rawValue === "" ? undefined : Math.trunc(Number(rawValue));
    setReceivedVnd((current) => ({ ...current, [personId]: Number.isSafeInteger(value) && value! >= 0 ? value : undefined }));
    setRun(undefined);
  }
...
function restoreReceivedVnd(facts: ApiFact[]): Record<string, number | undefined> { return Object.fromEntries(facts.flatMap((fact) => fact.subject && fact.predicate === "testamentary-share-received-vnd" && typeof fact.value === "number" ? [[fact.subject, fact.value] as const] : [])); }
function buildReceivedVndFacts(values: Record<string, number | undefined>): ApiFact[] { return Object.entries(values).flatMap(([personId, value]): ApiFact[] => value === undefined ? [] : [{ id: `${personId}-testamentary-vnd`, subject: personId, predicate: "testamentary-share-received-vnd", value }]); }
