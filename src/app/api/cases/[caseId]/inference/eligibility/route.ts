import { caseIdSchema, runEligibilitySchema } from "@/domain/case";
import { runStoredEligibility } from "@/server/cases/service";
...
export async function POST(request: Request, { params }: { params: Promise<{ caseId: string }> }) {
  ...
  const body = runEligibilitySchema.safeParse(await readJson(request));
  ...
  try {
    return Response.json(await runStoredEligibility(new CaseRepository(getDatabase()), caseId.data), { status: 201 });
  } catch (error) {
    const known = databaseErrorResponse(error);
    if (known) return known;
    console.error("Stored eligibility inference failed", error);
