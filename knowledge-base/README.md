...
Mô-đun `estate-settlement` triển khai R-I01 theo Điều 658 và R-I02 theo Điều 659 khoản 1. Mười cặp `obligation-type → priority` được lưu trong `payment-priority-knowledge`; một production rule tổng quát đối chiếu từng khoản. Với chia theo di chúc, mỗi `testamentary-distribution-group` có danh sách beneficiary riêng, completeness marker, trạng thái di chúc đã xác định phần và thỏa thuận khác. Không có observation không bị diễn giải thành `false`. Presenter cho phép chuyển giữa hai không gian tương tác. Hệ thống chưa phân bổ tài sản hoặc tính phần end-to-end.
